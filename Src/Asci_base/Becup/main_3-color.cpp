...
// Изменили char на std::string, чтобы буфер корректно хранил UTF-8 символы Юникода
struct Pixel {
    std::string symbol = " ";
...
// Возвращаем сплошную растеризацию (заливку полигона)
void rasterizeTriangleSolid(Triangle t, Pixel pix, std::vector<Pixel>& screen, std::vector<float>& zBuffer) {
...
// Твоя палитра Юникода высокой плотности
std::string getUnicodeChar(float brightness) {
    std::vector<std::string> unicodeChars = {" ", "░", "▒", "▓", "█"};
    int index = static_cast<int>(brightness * (unicodeChars.size() - 1));
    return unicodeChars[std::max(0, std::min(index, (int)unicodeChars.size() - 1))];
}
...
int main() {
    ...
    scene.push_back({ generateTorus(1.0f, 0.4f, 15, 30), {0.0f, 0.0f, 0.0f}, 0, 0, 0.02f, 0.04f, "\x1b[32m" }); // Зеленый
    scene.push_back({ generateCube(1.2f), {-3.2f, 0.0f, 0.0f}, 0, 0, 0.03f, 0.02f, "\x1b[31m" });              // Красный
    scene.push_back({ generateSphere(0.8f, 12, 24), {3.2f, 0.0f, 0.0f}, 0, 0, 0.01f, 0.05f, "\x1b[36m" });      // Бирюзовый

    float cameraOrbitAngle = 0.0f;
    float cameraRadius = 8.0f;    
    float zoomScale = 125.0f;     

    // ДВА ИСТОЧНИКА СВЕТА для мягких теней на Юникоде
    Vec3 lightDir1 = {0.2f, -0.6f, -1.0f};  // Ключевой
    lightDir1.normalize();
    Vec3 lightDir2 = {-0.6f, 0.6f, 1.0f};   // Заполняющий сзади
    lightDir2.normalize();
    ...
    while (true) {
        ...
        for (auto& model : scene) {
            ...
            for (const auto& tri : model.mesh) {
                ...
                float totalIntensity = intensity1 * 0.75f + intensity2 * 0.25f; 
                
                Pixel pix;
                pix.symbol = getUnicodeChar(totalIntensity); // Получаем блок Юникода
                ...
                rasterizeTriangleSolid(projected, pix, screenBuffer, zBuffer);
            }
        }
        ...
        for (int y = 0; y < HEIGHT; ++y) {
            for (int x = 0; x < WIDTH; ++x) {
                ...
                output += screenBuffer[idx].symbol; // Выводится строка-символ Юникода
            }
            output += '\n';
        }
        output += "\x1b[0m"; 
